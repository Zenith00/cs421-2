...
arbCallStmt = do
  ...
  argList <- resize 6 $ listOf $ arbIntOpExp 2
  let (argExpList, argValList)
        = unzip $ map (\(ExpValUnit e v) -> (e,v)) argList
      result@(_, pEnv', _) = aFun argValList
  case aProc of
    ProcedureStmt name _ _->
      return $ StmtEnvPEnvResultUnit
      { stmt = CallStmt name  argExpList
      , env = H.empty
      , penv = pEnv'
      , stmtResult = result
      }
