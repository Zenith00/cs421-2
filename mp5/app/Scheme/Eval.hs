...
eval expr@(List lst) = evalList $ map flattenList lst where
    ...
    evalList [Symbol "let", List(pairs), body] = 
        do env <- get
           pairs <- H.fromList <$> mapM getBinding pairs
    ...
    -- let*
    -- TODO: Handle `let*` here. Use pattern matching to match the syntax
    evalList [Symbol "let*", List([]), body] = 
        do v <- eval body
           return v

    evalList [Symbol "let*", List (x:xe), body] = 
        do env <- get
           (s, v) <- getBinding x
           modify $ H.insert s v
    ...
    -- the syntax
    evalList [Symbol "define-macro", List (Symbol fname : args), exp] = 
          do val <- (\argVal -> Macro argVal exp) <$> mapM getSym args
             modify $ H.insert fname val
             return Void
